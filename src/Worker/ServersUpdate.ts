...
const getApiKeys = (): Array<string> => {
    let i = 1;
    let currentHToken: string | undefined = process.env[`HETZNER_TOKEN_${i}`];
        ...
        for (let i = 0; i < hTokens.length; i++) {
            ...
            for (let i = 0; i < servers.length; i++) {
                ...
                const startDate = new Date(
                    (new Date() as any) -
                        parseFloat(
                            process.env.SERVER_METRICS_PERIOD as string
                        ) *
                            60000
                ).toISOString();
                ...
                if (!resNetwork) {
                    Logger.error("resNetwork not defined");
                    return;
                }
                ...
                const msg: any = new Discord.MessageEmbed()
                    .setColor(color)
